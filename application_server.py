...
myclient = MongoClient("mongodb://52.68.108.247:27017/")
...
class Server(object):
    ...
    def __init__(self, ip, port):
        ...
        self.sock.listen(5)
        self.known = ['invite', 'list-invite', 'accept-invite', 'list-friend', 'post', 'receive-post',\
                        'send', 'create-group', 'list-group', 'list_joined', 'join-group', 'send-group']
        conn = stomp.Connection10([('52.68.108.247',self.POST)])
        conn.start()
        conn.connect()
        queue_name = self.queue_name+'app2login'
        conn.send(queue_name, 'ok')
        conn.disconnect()
        ...
        while True:
            ...
            if not msg:
                ...
                try:
                    ...
                    mytoken = None
                try:
                    if mytoken == None and any(item in self.known for item in cmd):
